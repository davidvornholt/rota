...
Rota keeps a wardrobe of photographed garments and decides each morning what to wear. The rotation engine reads the wear log to identify clothes with days remaining and ranks alternatives for every slot. Gemini sees both, with their pictures. Rotation is a preference: weather and the occasion note can justify replacing any or all of yesterday's outfit. The wearer confirms with one tap, asks again, swaps one item, or logs something else entirely; whatever is logged becomes the truth the next day is built on.

"Pick again" excludes the fresh picks and retains yesterday's clothes as preferences when they suit today. "Reopen everything" excludes the continuing pieces too. Rejected garments return only when a required slot has no other eligible choice. The response schema limits each slot to its offered garment aliases, and the answer decoder rejects unknown, misplaced or duplicate garments.

Saving a note persists it first and regenerates a pending proposal. The current outfit remains pending until its replacement is inserted successfully. After a failed request, the page refreshes stored state so a saved note or completed outfit is visible, and a new attempt clears earlier action errors. Proxy and network failures get a recovery message before TanStack attempts to decode the response.
...
Proposal generation, note changes and outfit settlement share one process-local semaphore. A queued morning request checks for an existing proposal after acquiring it, preventing competing browser and scheduler generations. Each operation has a six-minute deadline including its queue wait, forecast, images, model retries and writes. Outfit model attempts allow five minutes at HIGH reasoning. Cancellation reaches Gemini through its abort signal. Server functions have a seven-minute request deadline, with Bun's idle timer disabled while handling them. Other requests use the configured idle timeout. The semaphore does not coordinate multiple server replicas.
...
Studio renders share two deployment slots per server process. Source loading and rotation run only after a job acquires a slot, so queued jobs do not retain prepared image buffers. Each job prepares its source once and reuses it across rate-limit retries and the transparency fallback. HTTP 429 responses wait for `retry-after-ms` or `Retry-After`, including HTTP dates, with a maximum ten-minute cooldown. Missing or invalid delays use exponential backoff starting at two seconds with jitter. Each background variant allows three retries. Accepted jobs wait for a slot without a queue deadline, so large batches can finish. Source preparation, cooldowns, and transparency fallback share a ten-minute active rendering budget. Saving edits before regeneration and storing the finished image each have a separate two-minute deadline. Recording a failure has its own thirty-second limit so a stalled write cannot prevent another attempt. Each individual image request has a four-minute timeout that aborts its fetch. Other failures are not automatically resubmitted.
...
Garment colours store only a validated `hex` value. `src/shared/data/color-name.ts` derives display names from the nearest of 24 wardrobe references by RGB distance. The picker, accessible swatch labels, AI prompts, and history grouping share this function. The database migration removes stored colour names before the updated app starts. Validation rejects colour objects with the removed `name` field. Names are approximate categories, while swatches and rendering instructions retain the exact colour value.

Small secondary actions use `src/shared/ui/icon-button.tsx`: a 44px target with an accessible name and a tooltip on hover or focus. Escape dismisses the tooltip before its containing dialog. Colour/layer removal, dialog close, existing-note editing, and the first step of discard/delete share this control; destructive confirmation stays explicit text.
