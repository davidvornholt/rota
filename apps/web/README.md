...
Proposal generation, note changes and outfit settlement share one process-local semaphore. A queued morning request checks for an existing proposal after acquiring it, preventing competing browser and scheduler generations. Each operation has a 180-second deadline including its queue wait, forecast, images, model retries and writes. Cancellation reaches Gemini through its abort signal. Server-function connections allow 240 seconds of inactivity so Bun can deliver the result or timeout error. Other requests keep Bun's default idle timeout. The semaphore does not coordinate multiple server replicas.
