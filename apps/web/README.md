...
Studio renders share two image slots per server process, and both slots share provider cooldowns. HTTP 429 responses wait for `retry-after-ms` or `Retry-After`, including HTTP dates, with a maximum ten-minute cooldown. Missing or invalid delays use exponential backoff starting at two seconds with jitter. Each background variant allows three retries. Waiting for a free slot and rendering each have a separate ten-minute budget; an overall job guard of twenty-two minutes also covers preparation and image storage, and client polling allows a further margin to observe completion or a recorded timeout. Recording a failure has its own thirty-second limit so a stalled write cannot prevent another attempt. Each individual image request has a four-minute timeout that aborts its fetch. Other failures are not automatically resubmitted.
