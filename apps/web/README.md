...
Studio renders share one deployment permit per server process. HTTP 429 responses wait for `retry-after-ms` or `Retry-After`, including HTTP dates. Missing or invalid delays use exponential backoff starting at two seconds with jitter. Each background variant allows three retries; the ten-minute overall deadline includes queueing, cooldowns, and the transparency fallback. Each individual request has a four-minute timeout that aborts its fetch. Other failures are not automatically resubmitted.

A garment has at most one extraction/render/storage job in the garments runtime. Repeated requests reuse that job. The wardrobe and detail page poll its queued, rendering, and waiting states without resetting unsaved form edits. Render errors are stored separately in `garment.studio_error`; they preserve the garment's lifecycle status and existing images. Provider diagnostics stay in server logs, while the interface offers a manual retry after failure.
